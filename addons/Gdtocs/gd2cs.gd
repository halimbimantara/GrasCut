...
func get_regex_container():
	return popup.get_node(
		"VBoxContainer/TopBottomSplit/LeftRightSplit/Panel_R/NameRepl/NameReplace"
	)
...
func do_conversion():
	...
	for file in files:
		if ProjectSettings.globalize_path(file[0]) == ProjectSettings.globalize_path(file[1]):
			print_to_console(
				(
					"Error: Input file path is equivalent to output file path. "
					+ file[0]
					+ " . SKIPPED!"
				),
				Color.red
		...
		var params = ["addons/Gdtocs/gd2cs.py", "-i", file[0], "-o", file[1]]
		if rename_funcs:
...
func add_input():
	...
	for child in result:
		var child_node: Node = load("res://addons/Gdtocs/scenes/FileElement.tscn").instance()
...
func add_rename_rule():
	...
	var child_node: Node = load("res://addons/Gdtocs/scenes/ReplaceElement.tscn").instance()
...
func install_regex():
	run_os_code(
		ProjectSettings.get_setting("gd2cs/config/python"), ["-m", "pip", "install", "regex"]
	)
...
func callback(ud):
	if not popup:
		popup = load("res://addons/Gdtocs/scenes/Editor_UI.tscn").instance()
		add_child(popup)
		...
		popup.find_node("NoRegexDialog").find_node("InstallRegex").connect(
			"pressed", self, "install_regex"
		)
